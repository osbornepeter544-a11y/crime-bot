...
class EscalationBuilder:
    ...
    def build(cls, case: Dict) -> str:
        """
        Construct the escalation paragraph with improved cinematic pacing.
        """
        ...
        # Key event 2 with tension transition
        if len(key_events) > 1:
            second_event = cls.clean_sentence(key_events[1])
            parts.append("Then, " + second_event.lower() + ".")
        ...
        # Second unresolved element (shorter punch)
        if len(unresolved) > 1:
            second_unresolved = cls.clean_sentence(unresolved[1])
            parts.append(second_unresolved + ".")
